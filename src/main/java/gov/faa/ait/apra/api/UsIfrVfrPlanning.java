...
public class UsIfrVfrPlanning extends AbstractTableDataService {
	...
	@Path("/chart")
    @Operation(summary="Get IFR planning download link by edition and format", 
	...
	public Response getIfrPlanningChart(
			@Parameter(name="edition", description="Requested product edition. If omitted, the default current edition is returned.", required=false)  @QueryParam("edition") String edition, 
			@ApiParam (name="format", description="Format of the requested chart. TIFF is georeferenced and PDF is not georeferenced. If omitted, the default PDF format is returned.", required=false) @QueryParam("format") String format) {
	...
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON, MediaType.TEXT_XML })
	@Path("/info")
    @Operation(summary="Get Planning Chart edition date and edition number by edition type", 
    	response=ProductSet.class)
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = RESPONSE_200),
			@ApiResponse(responseCode = "400", description = ERROR_400),
	...
	public Response getIfrPlanningInfo(
			@Parameter(name="edition", description="Requested product edition. If omitted, the default current edition information is returned.", required=false) 
